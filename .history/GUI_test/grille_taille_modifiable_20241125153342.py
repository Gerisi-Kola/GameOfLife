...
grid_size = tk.IntVar(value=5)

# Figure Matplotlib initiale
fig, ax = create_grid(grid_size.get())
...
def update_grid():
    ...
    for i in range(grid_size.get()):
        for j in range(grid_size.get()):
            rect = plt.Rectangle((j, i), 1, 1, fill=False, edgecolor='black')
            ax.add_patch(rect)
    
    ax.set_xlim(0, grid_size.get())
    ax.set_ylim(0, grid_size.get())
    ax.set_aspect('equal')
    ax.set_xticks([])
    ax.set_yticks([])
